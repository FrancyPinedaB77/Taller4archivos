...
for pregunta in trae_todo_coleccion: 			#Llega la consulta
	...
	entidad_organizacion=pregunta.get("entidades")[3]["ORGANIZATION"]
	if len(entidad_organizacion)!=0:
		x=0
		for x in range (len(entidad_organizacion)):
			kk=l+str(x)
			laorganizacion=entidad_organizacion[x][kk].title()
			x=x+1
 			texto_marcador= laorganizacion+" "+ "Pregunta #"+str(id_pregunta) + ":" + titulo_pregunta
 			#print texto_marcador
	 		try :
	 			sparql.setQuery("""
	 		    PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
	 		    SELECT ?lat
	 		    WHERE { <http://dbpedia.org/resource/"""+laorganizacion+"""> geo:lat ?lat }
	 			""")
	 			...
	 			for prueba_que_tenga in results["results"]["bindings"]:
	 				if len(prueba_que_tenga)==1:
	 					...
	 					sparql.setQuery("""PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
	 					    SELECT ?long
	 					    WHERE { <http://dbpedia.org/resource/"""+laorganizacion+"""> geo:long ?long }
	 					""")
						sparql.setReturnFormat(JSON)
						results = sparql.query().convert()
				...
				#print "aqui empieza la otra pregunta"
